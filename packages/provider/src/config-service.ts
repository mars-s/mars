...
export class ProviderConfigService implements ProviderSource<ProviderConfigSnapshot> {
  ...
  async savePersonalProviderOverlay(
    ...
    return this.#updatePersonal((current) => {
      ...
      // The two zhipu-account guards that used to sit here (reject disabling an
      // account provider, reject writing access onto one) are gone with the
      // account-provider subsystem. Every personal-provider guard below stays.
