...
export class ProviderRuntime {
  ...
  constructor(dependencies: ProviderRuntimeDependencies) {
    ...
    this.#disposeBuiltinRecovery = this.#configRuntime.onDidCheckZCodeBuiltin(async () => {
      ...
      await this.registryService.refresh("builtin-recovery");
    });
    // The account-provider overlay is gone, so the registry reads the personal
    // config alone. It keeps its own fail-closed empty account snapshot, which leaves
    // an account-backed provider unentitled rather than half configured.
