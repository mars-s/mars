...
async function readSettingsWithMeta(): Promise<ReadSettingsResult> {
  ...
  try {
    ...
    // A persisted providerFamilyDomain from a pre-removal install is simply absent from
    // the schema now; Zod's strip behaviour drops it from the parsed result, so the very
    // next successful write garbage-collects it from the settings blob. No explicit
    // clearing step is required or wanted here.
...
/**
 * The account-provider subsystem is deleted, so the legacy account-connection import
 * that used to be exposed here (and to wait on an OAuth organisation lookup) is gone.
 * The function stays so the `service` field keeps its call shape; it simply returns the
 * bare service.
 */
      ...
      const runUpdate = async (shouldCommit: () => boolean, enterCommitPhase: () => void) => {
        ...
        if (
          ...
          JSON.stringify(current.providerFamilyConnectionSelections ?? {}) !==
            JSON.stringify(expectedAccountSettings.providerFamilyConnectionSelections ?? {})
        ) {
