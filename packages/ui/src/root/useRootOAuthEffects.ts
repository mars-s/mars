...
async function handleOAuthCallbackSuccess(params: {
  ...
  // The provider family selection refresh is gone with the family layer; the login state and
  // the provider registry are still refreshed so the model picker reflects the new session.
  ...
  void reportAppTelemetryEvent(
    params.platform,
    {
      ...
      eventType: "view",
      eventExtraDetail: {
        login_provider: params.result.provider,
      },
    },
    "Root",
  );
...
export function useRootOAuthEffects({
  ...
  useEffect(() => {
    ...
    async function restoreOAuthSessionInBackground() {
      ...
      try {
        // OAuth token 生命周期较短，启动时如果仍走远端校验，
