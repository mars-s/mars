...
export function registerRemoteIpcHandlers(options: {
  ...
  ipcMain.on(PlatformChannels.OAuthCallbackHandled, (event) => {
    options.onOAuthCallbackHandled({ rendererId: event.sender.id });
  });

  // 业务事件上报已被移除。这里保留一个显式丢弃端点，让仍在调用方的 renderer
  // 走同一条无副作用路径，而不会因为缺少 handler 拿到一个 rejected invoke。
  ipcMain.handle(PlatformChannels.DiscardTelemetryEvent, async () => {});
